package com.stanco.entity;

import jakarta.persistence.*;
import lombok.AllArgsConstructor;
import lombok.Data;
import lombok.NoArgsConstructor;

import java.math.BigDecimal;
import java.time.LocalDate;
import java.time.LocalDateTime;

@Entity
@Table(name = "candidate_details")
@Data
@NoArgsConstructor
@AllArgsConstructor
public class CandidateDetails {

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long id;

    @Column(name = "cdID", nullable = false)
    private String cdID;

    @Column(name = "rfh_no", nullable = false)
    private String rfhNo;

    @Column(nullable = false, columnDefinition = "TEXT")
    private String status;

    @Column(name = "hepl_recruitment_ref_number")
    private String heplRecruitmentRefNumber;

    @Column(name = "candidate_name", nullable = false)
    private String candidateName;

    @Column(name = "candidate_cv", nullable = false, columnDefinition = "TEXT")
    private String candidateCv;

    @Column(name = "red_flag_status", nullable = false)
    private Integer redFlagStatus;

    @Column(name = "created_on", nullable = false)
    private String createdOn;

    @Column(name = "created_by", nullable = false)
    private String createdBy;

    @Column(name = "created_at")
    private LocalDateTime createdAt;

    @Column(name = "updated_at")
    private LocalDateTime updatedAt;

    @Column(name = "candidate_source", nullable = false)
    private String candidateSource;

    @Column(name = "gender", nullable = false)
    private String gender;

    @Column(name = "candidate_status", nullable = false)
    private Integer candidateStatus;

    @Column(name = "candidate_email", nullable = false)
    private String candidateEmail;

    @Column(name = "candidate_onborded_date")
    private LocalDate candidateOnbordedDate;

    @Column(name = "invoice_details", columnDefinition = "TEXT")
    private String invoiceDetails;

    @Column(name = "candidate_exp_status")
    private String candidateExpStatus;

    @Column(name = "candidate_type", nullable = false)
    private String candidateType;

    @Column(name = "candidate_mobile", nullable = false)
    private String candidateMobile;

    @Column(name = "proof_of_identity", nullable = false, columnDefinition = "TEXT")
    private String proofOfIdentity;

    @Column(name = "poi_filename", nullable = false, columnDefinition = "TEXT")
    private String poiFilename;

    @Column(name = "proof_of_address", nullable = false, columnDefinition = "TEXT")
    private String proofOfAddress;

    @Column(name = "poa_filename", nullable = false, columnDefinition = "TEXT")
    private String poaFilename;

    @Column(name = "tax_entity_proof", nullable = false, columnDefinition = "TEXT")
    private String taxEntityProof;

    @Column(name = "proof_of_relieving", nullable = false, columnDefinition = "TEXT")
    private String proofOfRelieving;

    @Column(name = "proof_of_vaccination", nullable = false, columnDefinition = "TEXT")
    private String proofOfVaccination;

    @Column(name = "proof_of_dob", nullable = false, columnDefinition = "TEXT")
    private String proofOfDob;

    @Column(name = "proof_of_bg", nullable = false, columnDefinition = "TEXT")
    private String proofOfBg;

    @Column(name = "proof_of_bankacc", nullable = false, columnDefinition = "TEXT")
    private String proofOfBankacc;

    @Column(name = "doc_status", nullable = false)
    private Integer docStatus;

    @Column(name = "offer_rel_status", nullable = false)
    private Integer offerRelStatus;

    @Column(name = "c_doc_status", nullable = false)
    private String cDocStatus;

    @Column(name = "payroll_status", nullable = false)
    private Integer payrollStatus;

    @Column(name = "payroll_remark", nullable = false, columnDefinition = "TEXT")
    private String payrollRemark;

    @Column(name = "po_letter_filename", nullable = false, columnDefinition = "TEXT")
    private String poLetterFilename;

    @Column(name = "leader_status", nullable = false)
    private Integer leaderStatus;

    @Column(name = "ld_reject_type", nullable = false)
    private String ldRejectType;

    @Column(name = "ld_reject_remark", nullable = false, columnDefinition = "TEXT")
    private String ldRejectRemark;

    @Column(name = "po_finance_status", nullable = false)
    private Integer poFinanceStatus;

    @Column(name = "fn_po_remark", nullable = false, columnDefinition = "TEXT")
    private String fnPoRemark;

    @Column(name = "fn_po_attach", nullable = false, columnDefinition = "TEXT")
    private String fnPoAttach;

    @Column(name = "payroll_verify_type", nullable = false)
    private String payrollVerifyType;

    @Column(name = "welcome_buddy", nullable = false)
    private String welcomeBuddy;

    @Column(name = "offer_letter_filename", nullable = false, columnDefinition = "TEXT")
    private String offerLetterFilename;

    @Column(name = "or_cc_mailid", nullable = false)
    private String orCcMailid;

    @Column(name = "or_bc_mailid", nullable = false)
    private String orBcMailid;

    @Column(name = "or_doj")
    private LocalDate orDoj;

    @Column(name = "closed_salary_pa", nullable = false)
    private Integer closedSalaryPa;

    @Column(name = "last_drawn_ctc", nullable = false)
    private Integer lastDrawnCtc;

    @Column(name = "register_type", nullable = false)
    private String registerType;

    @Column(name = "register_type_per")
    private String registerTypePer;

    @Column(name = "get_emp_mode", nullable = false)
    private String getEmpMode;

    @Column(name = "or_department", nullable = false)
    private String orDepartment;

    @Column(name = "po_type", nullable = false)
    private String poType;

    @Column(name = "po_file_status", nullable = false)
    private Integer poFileStatus;

    @Column(name = "client_po_attach", nullable = false, columnDefinition = "TEXT")
    private String clientPoAttach;

    @Column(name = "client_po_number", nullable = false)
    private String clientPoNumber;

    @Column(name = "or_recruiter_name", nullable = false)
    private String orRecruiterName;

    @Column(name = "or_recruiter_email", nullable = false)
    private String orRecruiterEmail;

    @Column(name = "or_recruiter_mobile_no", nullable = false)
    private String orRecruiterMobileNo;

    @Column(name = "approver", nullable = false)
    private String approver;

    @Column(name = "c_doc_upload_status", nullable = false)
    private Integer cDocUploadStatus;

    @Column(name = "proof_of_attach", nullable = false, columnDefinition = "TEXT")
    private String proofOfAttach;

    @Column(name = "client_po_update_date", nullable = false)
    private String clientPoUpdateDate;

    @Column(name = "client_po_value")
    private String clientPoValue;

    @Column(name = "client_po_validity")
    private String clientPoValidity;

    @Column(name = "esi_type")
    private String esiType;

    @Column(name = "middle_name")
    private String middleName;

    @Column(name = "last_name")
    private String lastName;

    @Column(name = "dob")
    private String dob;

    @Column(name = "age")
    private String age;

    @Column(name = "marital_status")
    private String maritalStatus;

    @Column(name = "blood_group")
    private String bloodGroup;

    @Column(name = "attendance_format")
    private String attendanceFormat;

    @Column(name = "weak_off")
    private String weakOff;

    @Column(name = "payroll_status_ctc")
    private String payrollStatusCtc;

    @Column(name = "vertical")
    private String vertical;

    @Column(name = "onboarder")
    private String onboarder;

    @Column(name = "reviewer")
    private String reviewer;

    @Column(name = "primary_reporter")
    private String primaryReporter;

    @Column(name = "additional_reporter")
    private String additionalReporter;

    @Column(name = "onboard_status", nullable = false)
    private Integer onboardStatus;

    @Column(name = "client_type")
    private String clientType;

    @Column(name = "payroll_status_head_approve", nullable = false)
    private Integer payrollStatusHeadApprove;

    @Column(name = "offer_release_remarks", columnDefinition = "TEXT")
    private String offerReleaseRemarks;

    @Column(name = "director_status", nullable = false)
    private Integer directorStatus;

    @Column(name = "preferred_location")
    private String preferredLocation;

    @Column(name = "company_name")
    private String companyName;

    @Column(name = "designation")
    private String designation;

    @Column(name = "total_experience")
    private String totalExperience;

    @Column(name = "relevant_experience")
    private String relevantExperience;

    @Column(name = "current_ctc")
    private String currentCtc;

    @Column(name = "expected_ctc")
    private String expectedCtc;

    @Column(name = "notice_period")
    private String noticePeriod;

    @Column(name = "offers_in_hand")
    private String offersInHand;

    @Column(name = "screening_comments", columnDefinition = "TEXT")
    private String screeningComments;

    @Column(name = "remarks", columnDefinition = "TEXT")
    private String remarks;

    @Column(name = "availability")
    private String availability;

    @Column(name = "l1_interview_date")
    private LocalDate l1InterviewDate;

    @Column(name = "l2_interview_date")
    private LocalDate l2InterviewDate;

    @Column(name = "l3_interview_date")
    private LocalDate l3InterviewDate;

    @Column(name = "hr_discussion_date")
    private LocalDate hrDiscussionDate;

    @Column(name = "follow_up_date")
    private LocalDate followUpDate;

    @Column(name = "margin")
    private BigDecimal margin;

    @Column(name = "billing_value")
    private BigDecimal billingValue;

    @Column(name = "feedback_notes", columnDefinition = "TEXT")
    private String feedbackNotes;
}